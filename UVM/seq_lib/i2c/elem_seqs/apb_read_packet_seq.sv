`ifndef APB_READ_PACKET_SEQ_SV
`define APB_READ_PACKET_SEQ_SV

class apb_read_packet_seq extends apb_base_sequence;

  typedef enum {NOT_EMPTY_THEN_READ, FULL_THEN_READ, DIRECTED_READ} apb_read_packet_style_t;

  rand apb_read_packet_style_t read_style = NOT_EMPTY_THEN_READ;

  `uvm_object_utils(apb_read_packet_seq)
  ...
  function new (string name = "apb_read_packet_seq");
    super.new(name);
  endfunction
