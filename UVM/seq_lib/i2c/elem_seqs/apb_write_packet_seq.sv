...
`ifndef APB_WRITE_PACKET_SEQ_SV
`define APB_WRITE_PACKET_SEQ_SV

class apb_write_packet_seq extends apb_base_sequence;

  `uvm_object_utils(apb_write_packet_seq)

  constraint def_cstr {
    soft packet.size() == 1;
  }

  function new (string name = "apb_write_packet_seq");
    super.new(name);
  endfunction
