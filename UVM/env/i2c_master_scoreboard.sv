`ifndef I2C_MASTER_SCOREBOARD_SV
`define I2C_MASTER_SCOREBOARD_SV
...
class i2c_master_scoreboard extends uvm_component;

  i2c_config cfg;
  // TODO
  // Analysis import declarion below
  uvm_analysis_imp_apb_master #(lvc_apb_transfer, i2c_master_scoreboard) apb_trans_observed_imp;
  uvm_analysis_imp_i2c_slave #(lvc_i2c_slave_transaction, i2c_master_scoreboard) i2c_trans_observed_imp;
  ...
  task i2c_refmod();
    ...
    lvc_apb_transfer tr;
    ral_reg_i2c_IC_DATA_CMD data_cmd_r;
    bit[7:0] data;                             // define

    data_cmd_r = new("data_cmd_r");
    data_cmd_r.build();                        // reg model

    forever begin
      wait(apb_trans_observed.size() > 0)      //------------ monitor

      tr = apb_trans_observed.pop_front();
...
`endif // I2C_MASTER_SCOREBOARD_SV
