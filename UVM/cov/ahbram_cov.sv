...
class ahbram_cov extends ahbram_subscriber;
  ...
  covergroup ahbram_t1_address_cg(bit [31:0] addr_start, bit [31:0] addr_end) with function sample(bit [31:0] addr);
  ...
  covergroup ahbram_t2_type_size_cg with function sample(burst_type_enum btype, burst_size_enum bsize);
  ...
  function new (string name = "ahbram_cov", uvm_component parent);
    ...
    ahbram_t1_address_cg = new(32'h0000, 32'hFFFF);
    ahbram_t2_type_size_cg = new();
  endfunction
  ...
  function void write(lvc_ahb_transaction tr);
    ahbram_t1_address_cg.sample(tr.addr);
    ahbram_t2_type_size_cg.sample(tr.burst_type, tr.burst_size);
  endfunction
