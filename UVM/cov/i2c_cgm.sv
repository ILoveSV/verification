...
// Coverage Model
class i2c_cgm extends uvm_component;

  // Analysis import declarion below
  uvm_analysis_imp_apb_master #(lvc_apb_transfer, i2c_cgm) apb_trans_observed_imp;
  uvm_analysis_imp_i2c_slave #(lvc_i2c_slave_transaction, i2c_cgm) i2c_trans_observed_imp;

  // Events
  uvm_event i2c_field_access_bd_e; //  register field backdoor access event
  uvm_event i2c_field_access_fd_e; //  register field frontdoor access event
  ...
  function new(string name = "i2c_cgm", uvm_component parent = null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    i2c_field_access_bd_e = uvm_event_pool::get_global("i2c_field_access_bd_e");
    i2c_field_access_fd_e = uvm_event_pool::get_global("i2c_field_access_fd_e");

    if(!uvm_config_db #(i2c_config)::get(this, "", "cfg", cfg)) begin
      `uvm_error("build_phase", "Unable to get i2c_config from uvm_config_db")
    end
    rgm = cfg.rgm;
    vif = cfg.vif;
