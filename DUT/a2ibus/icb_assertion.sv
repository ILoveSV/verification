...
// Signal X Assertion
    ...
    property icb_rsp_rdata_no_x_check;
        @(posedge icb.clk) disable iff(!icb.rst_n)
        icb.icb_rsp_valid |-> (!$isunknown(icb.icb_rsp_rdata));
    endproperty
...
// Signals keep while valid and no handshake
    ...
    // ... cmdread cmdwmask rsperr rsprdata rspvalid
...
property icb_cmd_wmask_keep_check;
    @(posedge icb.clk) disable iff(!icb.rst_n)
    (icb.icb_cmd_valid && !icb.icb_cmd_ready) |=> $stable(icb.icb_cmd_wmask);
endproperty
...
property icb_rsp_rdata_keep_check;
    @(posedge icb.clk) disable iff(!icb.rst_n)
    (icb.icb_rsp_valid && !icb.icb_rsp_ready) |=> $stable(icb.icb_rsp_rdata);
endproperty
...
// Handshake
    property icb_cmd_handshake_check;
        @(posedge icb.clk) disable iff(!icb.rst_n)
        (($rose(icb.icb_cmd_valid)) or ($past(icb.icb_cmd_valid && icb.icb_cmd_ready) && icb.icb_cmd_valid)) 
        |-> ##[0:$] icb.icb_cmd_valid && icb.icb_cmd_ready;
    endproperty
property icb_rsp_handshake_check;
    @(posedge icb.clk) disable iff(!icb.rst_n)
    (($rose(icb.icb_rsp_valid)) or ($past(icb.icb_rsp_valid && icb.icb_rsp_ready) && icb.icb_rsp_valid)) 
    |-> ##[0:$] icb.icb_rsp_valid && icb.icb_rsp_ready;
endproperty
